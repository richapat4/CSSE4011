...
class Controller:
    def __init__(self):
        ...
        self.separated_clusters = pd.DataFrame()
        self.num_clusters = 0
    ...
    def clustering(self, testData):
        ...
        total_cluster = pd.DataFrame({'X': [0], 'Y': [0], 'Z': [0], 'cluster_num': 0})
        ...
        if (len(testData) > 0):
            ...
            for cluster_label in range(n_clusters_):
                ...
                entry = pd.Series({'X': center_point['X'], 'Y': center_point['Y'], 'Z': center_point['Z'],
                                   'label': center_point['cluster_num']})
                center_df.loc[len(center_df)] = entry

            self.num_clusters = n_clusters_
            self.cluster_points = pd.concat([self.cluster_points, center_df], axis=0)
    ...
    def parseConfigFile(self, configFileName):
        ...
        configParameters["dopplerResolutionMps"] = 3e8 / (
                    2 * startFreq * 1e9 * (idleTime + rampEndTime) * 1e-6 * configParameters[
    ...
    def readAndParseData18xx(self, Dataport, configParameters):
        ...
        if self.byte_buffer_len > 16:
            ...
            if startIdx:
                ...
                if startIdx[0] > 0 and startIdx[0] < self.byte_buffer_len:
                    self.byte_buffer[:self.byte_buffer_len - startIdx[0]] = self.byte_buffer[
                                                                            startIdx[0]:self.byte_buffer_len]
                    self.byte_buffer[self.byte_buffer_len - startIdx[0]:] = np.zeros(
                        len(self.byte_buffer[self.byte_buffer_len - startIdx[0]:]), dtype='uint8')
        ...
        if magicOK:
            ...
            for tlvIdx in range(numTLVs):
                ...
                if tlv_type == MMWDEMO_UART_MSG_DETECTED_POINTS:
                    ...
                    for objectNum in range(numDetectedObj):
                        ...
                        entry = pd.Series(
                            {"X": x[objectNum], "Y": y[objectNum], "Z": z[objectNum], "Velocity": velocity[objectNum]})
            ...
            if idX > 0 and self.byte_buffer_len > idX:
                ...
                self.byte_buffer[self.byte_buffer_len - shiftSize:] = np.zeros(
                    len(self.byte_buffer[self.byte_buffer_len - shiftSize:]), dtype='uint8')
...
if __name__ == "__main__":
    ...
    # while len(gui.separated_clusters) == 0:
    #     time.sleep(1)
